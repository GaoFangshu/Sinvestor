...
class DataCompany:
    ...
    def gen_variables(self):
        ...
        self.dummy_class_first = pd.get_dummies(self.data_itjuzi['一级分类'], prefix='dummy_一级分类')  # 19836 rows x 18 columns
        ...
        self.dummy_invested = (self.data_itjuzi['获投时间2'] != '-').astype(int)  # whether have been invested before, 1 for yes
        ...
        self.year_from_establish = invest_days(get_nth_investment(1, '获投时间', 15, self.data_itjuzi),
                                          (self.data_itjuzi['注册时间'] + '')) / np.timedelta64(365, 'D')  # years from company's establishment
...
class DataInvestor:
    ...
    def gen_variables(self):
        ...
        self.dummy_inv_type['dummy_机构类型_战略投资者'] = self.dummy_inv_type['dummy_机构类型_战略投资者'] + \
                                                             self.dummy_inv_type['dummy_机构类型_VC/战略投资者']
        self.dummy_inv_type.drop(['dummy_机构类型_VCPE', 'dummy_机构类型_VC/PE', 'dummy_机构类型_VC/战略投资者'], axis=1)    # 10106 rows x 8 columns
        ...
        self.var_amount_invest = amount_invest.astype(float).replace(0, np.NaN).groupby(level=0).mean().replace(np.NaN, 0)
    ...
    def gen_data(self):
        ...
        self.variables_invjuzi = pd.concat([self.data_invjuzi['投资机构名称'], self.total_amount, self.CNY_amount,
                                            ...
                                            self.dummy_invround, self.percent_industry, self.percent_round], axis=1)    # 6607 rows x 83 columns
        self.variables_geshang = pd.concat([self.data_geshang['机构简称'], self.dummy_inv_type, self.dummy_cap_type,
                                            self.manage_money, self.num_fund, self.num_inv, self.num_quit, self.num_quit_inv,
                                            self.percent_inv_industry, self.percent_inv_period, self.percent_quit_industry,
